...
/** Returns the index of the best upcoming/today day within the full days array. */
function bestDayIndex(days: DayForecast[]): number {
  const todayStr = new Date().toISOString().split('T')[0]
  let best = 0
  let bestVis = -1
  for (let i = 0; i < days.length; i++) {
    // Skip historical days before today
    if (days[i].date < todayStr && !days[i].is_forecast) continue
    ...
    try {
      ...
      setConditions(prev => ({
        ...
        [loc.id]: {
          ...
          today,
          days: forecast.days,
          // bestDayIdx computed over the full days array so it aligns with the mini strip
          bestDayIdx: bestDayIndex(forecast.days),
          resolvedLat: lat,
          resolvedLon: lon,
        },
      }))

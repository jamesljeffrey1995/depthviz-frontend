...
### Tabs (in-page)
`src/components/Tabs.tsx` + `Tabs.module.css` — the same pill-segmented idiom as the top nav (`--surface-sunken` track, `--accent` fill on the active tab), reused as one shared component wherever a screen needs an in-page tab set (currently: Catches, Admin, Profile, Friends, Apnea tables). Introduced to replace five near-identical hand-copied `.tabs`/`.tab`/`.tabActive` implementations that had drifted from the token system independently — new in-page tab UIs should use this component rather than a sixth hand-rolled copy.
...
**Update:** the paragraph originally here claimed the token/color migration was complete app-wide after the first pass. It wasn't — a follow-up design critique found roughly 35 of ~43 component files still rendering literal, unconverted hex/rgba values from the *previous* dark-theme identity (not this pass's tokens), including a safety-critical contrast failure in the apnea-training warning copy. That gap has since been closed:

Every other screen (forum, news, apnea tables, catches, admin panels, map/spot popups, friends, profile, tides, feed, legal, weight calculator, dive-video analysis, ML/seabed admin tooling, etc.) has now had its literal legacy colors, radii, and font sizes replaced with the real tokens above — verified by re-running the detector against the full `src` tree, not just claimed. `CatchesPage`, `AdminPanel`, `FriendsPanel`, and the apnea-tables flow (`ApneaTablesPage`/`ApneaTableEditor`/`ApneaTableRunner`/`ShareTableModal`) additionally got a structural pass in that update: they now share one `Tabs` component (`src/components/Tabs.tsx`) instead of five hand-copied `.tabs`/`.tab`/`.tabActive` implementations, and their literal `←`/`+`/`×` glyphs were replaced with the icon set (`IconChevronLeft`, `IconPlus`, `IconClose`).

**Second update — the remaining secondary screens' compositional rework, flagged above as still outstanding, is now done too.** `NewsPage`, `TidesPage`, `FeedPage`, `LegalPage`, `WeightCalculator`, `ReportForm` (+ its embedded `VisibilityAnalyser`), `DisputeForm`, `BestVisibility`, `SpotsMap`, `PlacesDashboard`, `LocationHistory`, `MLCharts`, `SeabedEditor`, and `SavedPlaces` all moved from "flat, undifferentiated list/card" layouts to the flagship's actual spatial grammar: primary content on `--surface-raised` cards (`--radius-md`, `--shadow-card`), recessed sub-panels on `--surface-sunken`, and real rhythm contrast (tight `--space-xs`/`--space-sm` within a group, generous `--space-lg`/`--space-xl` between distinct sections) rather than one repeated gap value everywhere. Each screen now gives its one primary reading or action clear visual priority instead of flat equal weight — e.g. `TidesPage`'s current-state + range chart as one hero card over a demoted events list, `BestVisibility`'s top-ranked spot as a distinct winner card over the rest of the ranked list (deliberately using size/weight/elevation for this, not the `InstrumentGauge` component, which stays reserved for the real 0–15m reading per the Do/Don't list below), and `WeightCalculator`'s result as the unambiguous focal point with its fields grouped into named clusters instead of one flat list. `LocationHistory`'s dive log is now grouped by month instead of one undifferentiated list. This pass also caught and fixed a real severity-color registration bug in `BestVisibility` (`.decent` was mapped to `--sev-good` instead of `--sev-decent`, so decent-visibility spots displayed with the "good" color) and a `SavedPlaces.module.css` that the first update's summary claimed was "token-only" but had in fact never been touched — both are fixed now.

Two dark/non-chassis surfaces remain by deliberate design, not oversight: `DayDetail.module.css`'s `.debugPanel` (documented above, admin-only) and `ShareTableModal.module.css`'s modal backdrop scrim (`rgba(2, 13, 20, 0.88)` — a full-viewport dimming layer behind the share dialog, which conventionally stays a dark translucent overlay regardless of the host theme). Both are the only findings a full-repo detector scan still surfaces, and both are intentional.
